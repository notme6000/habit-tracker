...
if(isset($_POST['setpriority']) && $_POST['setpriority']=='Priority')
{
	foreach($_POST as $postKey => $postVal)
	{
		$postKeyArray=explode('_',$postKey);
		$ti=0;
		if($postKeyArray[0]=='pri')
		{
			if(mysqli_query($conn,"UPDATE galleries SET priority='$postVal' WHERE id='{$postKeyArray[1]}'"))
			{
				$ti++;
			}
		}
	...
	if($ti>0)
	{
	$_SESSION['error']="Priority Updated!";
	mysql_close($con);
...
$where='WHERE 1';
$pageArray=array();
$dataQuery=mysqli_query($conn,"select * from admin ".$where);
$dataCount=mysqli_num_rows($dataQuery);
if($dataCount)
{
$pagesize=$numSize;
...
for($i=0;$i<$pages;$i++)
{
array_push($pageArray,'"templateListAdmin.php?start='.$i.'&pagesize='.$pagesize.'&where='.$where.'&navigateid='.$navigateid.'"');
}
}
else
{
