...
if(isset($_SESSION['sess_admin_id']))
 ...
 {  
	    ...
	    $sql = "INSERT INTO expense_transaction (trdate,user_id, head_id, amount) VALUES ('$trdate','$user', '$head_id','$amount')";
		if(mysqli_query($conn,$sql))
		{
			$_SESSION['success']=$counts."Expense Transaction Added!";
			mysqli_close($conn);
...
<select  name="head_id" class="form-control input-lg" id="head_id" >
    ...
    <?php
    $query = "select * from expense_category where user_id='$user' order by expense_category";
    $results = mysqli_query($conn,$query);

    while ($rows = mysqli_fetch_assoc(@$results)){ 
    ?>
    <option value="<?php echo $rows['id'];?>"><?php echo $rows['expense_category'];?></option>
...
<div class="col-md-5 container">
    <div class="col-md-12">
          <div class="list-group" style="max-height:900px; overflow:scroll;">
					<h3>Expense</h3>
                    <?	
					  $query = "select * from expense_transaction where user_id='$user'";
					  ...
					  echo "<table>";
					   ...
					   while($row = mysqli_fetch_array($result)) 
					   {
						  ...
						  echo "<tr>";
						  echo "<td> $td </td>";
						  $qry=mysqli_query($conn,"select expense_category from expense_category where id='$row[head_id]'");
						  $cat=mysqli_fetch_assoc($qry);
						  echo "<td>   $cat[expense_category]</td>";
