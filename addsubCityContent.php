...
if(isset($_SESSION['sess_admin_id']))
  ...
  if ($_POST['submitForm'] == "yes")
 {
	if($id=='')
	{
	 ...
	 $subcity_name=addslashes($_POST['subcity_name']);
	 $city_content=addslashes($_POST['city_content']);
	if(mysql_query("INSERT INTO subcity SET city='$city_name',subcity='$subcity_name',city_content='$city_content'"))
		{
	...
	else
	{
		 ...
		 $subcity_name=addslashes($_POST['subcity_name']);
		 $city_content=addslashes($_POST['city_content']);
		if(mysql_query("UPDATE subcity SET city='$city_name',subcity='$subcity_name',city_content='$city_content' WHERE id='$id'"))
		{
...
elseif (trim($id) != "") 
{
	$sql="select * from subcity where id='$id'";
	$result=executequery($sql);
	$num=mysql_num_rows($result);
	if($line=ms_stripslashes(mysql_fetch_array($result))){
	@extract($line);
	$content=stripslashes(htmlspecialchars_decode($line['city_content']));
}
}
?>
...
<td align="left" colspan="3">
<input name="city_name" type="text" class="txtfld" id="city_name" value="<?=$city;?>" size="120" maxlength="120" /></td>
</tr>
<tr class="oddRow">
<td width="22%" align="right" class="txt">Subcity Name:</td>
<td align="left" colspan="3">
<input name="subcity_name" type="text" class="txtfld" id="subcity_name" value="<?=$subcity;?>" size="120" maxlength="120" /></td>
</tr>
<tr>
<tr class="oddRow" id="tr_page_title_gr">
<td width="30%" align="right" class="txt">Subcity Content:</td>
...
<?
if($content=='') {
	$sql="select * from pre_content where 1=1";
	$cresult=executequery($sql);
	$num=mysql_num_rows($cresult);
	if($cline=ms_stripslashes(mysql_fetch_array($cresult))){	
	$content = $cline['subcity_content'];
}
}
?>
<textarea name="city_content" rows="10" cols="100"><?=$content;?></textarea>
<script>
