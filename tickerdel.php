...
$str_rest_refs=implode(",",$arr);
	if($Submit=='Delete')
	{
		$sql="delete from ticker where id in ($str_rest_refs)"; 
		executeUpdate($sql);
	...
	{
		$sql="update ticker set status='A' where id in ($str_rest_refs)";
		executeUpdate($sql);
	...
	{
		$sql="update ticker set status='D' where id in ($str_rest_refs)"; 
		executeUpdate($sql);
