...
$str_rest_refs=implode(",",$arr);
	if($Submit=='Delete')
	{
		$sql="delete from admin where id in ($str_rest_refs)"; 
		executeUpdate($sql);
	...
	{
		$sql="update admin set status='1' where id in ($str_rest_refs)";
		executeUpdate($sql);
	...
	{
		$sql="update admin set status='0' where id in ($str_rest_refs)"; 
		executeUpdate($sql);
