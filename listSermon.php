...
<div style="width:68%; height:20px;" align="center">
	<form action="listSermon.php" method="post">
	  <select name="language"  id="lang_id" size="1">
...
$where="WHERE language='$lid'";
$pageArray=array();
$dataQuery=mysql_query("select * from sermon_audio ".$where);
...
for($i=0;$i<$pages;$i++)
{
array_push($pageArray,'"templateListSermon.php?start='.$i.'&pagesize='.$pagesize.'&where='.$where.'&navigateid='.$navigateid.'"');
}
}
else
{
