...
if(isset($_POST['setpriority']) && $_POST['setpriority']=='Priority')
{
	foreach($_POST as $postKey => $postVal)
	{
		...
		{
			if(mysql_query("UPDATE states SET priority='$postVal' WHERE id='{$postKeyArray[1]}'"))
			{
				$ti++;
			}
		}
...
$where='WHERE 1';
$pageArray=array();
$dataQuery=mysql_query("select * from states ".$where);
...
for($i=0;$i<$pages;$i++)
{
array_push($pageArray,'"templateListStates.php?start='.$i.'&pagesize='.$pagesize.'&where='.$where.'&navigateid='.$navigateid.'"');
}
}
else
{
