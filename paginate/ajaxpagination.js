...
ajaxpageclass.loadstatustext="<div style='width:200px;'><div style='width:25px;float:left;'><img src='img/icons/loader.gif'></div><div style='width:175px;'>Loading, Please wait...</div></div>" // HTML to show while requested page is being fetched:
...
ajaxpageclass.takeCookie=function(Name){ 
...
ajaxpageclass.getInitialPage=function(divId, pageinfo){
	var persistedpage=this.takeCookie(divId)
	var selectedpage=(this.paginatepersist && this.takeCookie(divId)!=null)? parseInt(this.takeCookie(divId)) : pageinfo.selectedpage
...
ajaxpageclass.createBook.prototype={
	...
	buildpagination:function(selectedpage){ //build pagination links based on length of this.pageinfo.pages[]
		...
		if (this.pageinfo.pages.length<=1){ //no 0 or just 1 page
			...
			paginateHTML+='<li><a href="#next" rel="'+(selectedpage+1)+'">&raquo;</a></li>\n' //next link HTML
			paginateHTML+='</ul></div>'
		}// end construction
