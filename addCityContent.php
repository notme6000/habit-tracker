...
if(isset($_SESSION['sess_admin_id']))
  ...
  if ($_POST['submitForm'] == "yes")
 {
	if($id=='')
	{
	 ...
	 $city_name_url=addslashes($_POST['city_name_url']);
	 $city_content=addslashes($_POST['city_content']);
	if(mysqli_query($conn,"INSERT INTO city_all SET city_name='$city_name',city_name_url='$city_name_url',city_content='$city_content'"))
		{
			$_SESSION['success']="City Content Added.";
			mysqli_close($conn);
			header("Location:".$getNavigationData['menu_url']."?id=".mysqli_insert_id());
			exit();
		}
		else
		{
	...
	else
	{
		 ...
		 $city_name_url=addslashes($_POST['city_name_url']);
		 $city_content=addslashes($_POST['city_content']);
		if(mysqli_query($conn,"UPDATE city_all SET city_name='$city_name',city_name_url='$city_name_url',city_content='$city_content' WHERE city_id='$id'"))
		{
...
elseif (trim($id) != "") 
{
	$sql="select * from city_all where city_id='$id'";
...
<tr class="oddRow">
<td width="13%" align="right" class="txt">City Name:</td>
<td width="87%" colspan="3" align="left">
<input name="city_name" type="text" class="txtfld" id="city_name" value="<?=$city_name;?>" size="120" maxlength="120" /></td>
</tr>
<tr class="oddRow">
<td width="13%" align="right" class="txt">City URL(prefix .html):</td>
<td align="left" colspan="3">
<input name="city_name_url" type="text" class="txtfld" id="city_name_url" value="<?=$city_name_url;?>" size="120" maxlength="120" /></td>
</tr>
<tr>
<tr class="oddRow" id="tr_page_title_gr">
<td width="13%" align="right" class="txt">City Content:</td>
...
//$oFCKeditor->Create() ;
?>
<textarea name="city_content" rows="10" cols="100"><?=stripslashes(htmlspecialchars_decode($line['city_content']));?></textarea>
<script>
